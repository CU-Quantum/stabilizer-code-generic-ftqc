...
class CompiledDecoderByMatrix(CompiledDecoder):
    ...
    def decode_shots_bit_packed(self, *, bit_packed_detection_event_data: np.ndarray) -> np.ndarray:
        ...
        predictions = np.zeros((unpacked.shape[0], int(np.ceil(len(self._observables) / 8))), dtype=np.uint8)
        if unpacked.max():
            ...
            for combo in possible_combos:
                ...
                indices_matching = np.where(np.all(unpacked[:, self._symplectic_matrix.shape[0]:] == found_syndrome, axis=1))[0]
