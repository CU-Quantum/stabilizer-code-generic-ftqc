...
class CompiledDecoderByMatrix(CompiledDecoder):
    ...
    def decode_shots_bit_packed(self, *, bit_packed_detection_event_data: np.ndarray) -> np.ndarray:
        unpacked = np.unpackbits(bit_packed_detection_event_data, axis=1, bitorder='little')[:, :-2]
        ...
        predictions = np.zeros((unpacked.shape[0], self._symplectic_matrix.shape[1]), dtype=np.uint8)
        for combo in possible_combos:
            ...
            if len(indices_matching):
                predictions[indices_matching] = [found_noise.copy() for _ in range(len(indices_matching))]
