...
class TestVerificationIsZero:
    def test_measurements_are_separate_per_instance(self):
        separate_condition_instances = [VerificationIsZero() for _ in range(2)]
        ...
        assert all(separate_condition_instance.last_num_measurements == 1 for separate_condition_instance in separate_condition_instances)

    def test_missing_key(self):
        store = ClassicalDataDictionaryStore()
        condition = VerificationIsZero()
