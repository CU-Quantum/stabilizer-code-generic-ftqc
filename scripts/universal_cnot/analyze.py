from cirq import Circuit, CircuitOperation, DEFAULT_RESOLVERS, FrozenCircuit, I, Moment, OP_TREE, Operation, \
    TaggedOperation, \
    map_moments, \
    read_json
...
def remove_operation(moment: Moment, moments_path: list[int], operation_to_remove: Operation) -> OP_TREE:
    ...
    if len(moments_path) == 1:
        ...
        return TaggedOperation(
            CircuitOperation(
                FrozenCircuit(
                    remove_operation(moment=moment, moments_path=moments_path,
...
if __name__ == '__main__':
    ...
    for i, errored_circuit_operation in enumerate(errored_circuit_operations):
        ...
        circuit_without_operation = Circuit(
            remove_operation(moment=moment,
        ...
        success = UniversalCnot.was_successful(measurements_per_shot=results.measurements_per_shot)
