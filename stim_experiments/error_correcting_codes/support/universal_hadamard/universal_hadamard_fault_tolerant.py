...
class UniversalHadamardFaultTolerant(UniversalHadamard):
    ...
    def _encode_helper_registers(self, context: UniversalHadamardFaultTolerantContext) -> OP_TREE:
    ...
    def _cx_original_to_helpers(self, context: UniversalHadamardFaultTolerantContext) -> OP_TREE:
    ...
    def _create_cat_states_on_all_subregisters(self, context: UniversalHadamardFaultTolerantContext) -> OP_TREE:
    ...
    def _encode_universal_hadamard_helper(self, context: UniversalHadamardFaultTolerantContext):
        subregister_to_cat = context.three_subregister_parity_codes_small[0].subregisters[0]
    ...
    def _use_fresh_ancilla_qubits(self) -> Generator[UniversalHadamardFaultTolerantContext, None, None]:
        ...
        with FreshAncillasPool().use_fresh_ancillas(num_ancillas=num_subregister_parity_codes * num_qubits_per_subregister_parity_code) as ancilla_qubits:
            ...
            universal_hadamard_helper_code = UniversalHadamardHelperCode(
                num_qubits_in_cat_state=len(three_subregister_parity_codes[0].subregisters[0]),
                qubits=three_subregister_parity_codes[0].data_qubits
            )
            logical_x, logical_z = (
