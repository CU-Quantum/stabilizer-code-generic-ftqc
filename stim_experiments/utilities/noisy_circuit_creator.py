...
from stim_experiments.error_correcting_codes.support.operations_applier.operations_applier_using_cat_state.operations_applier_using_cat_state import \
    OPERATIONS_APPLIER_USING_CAT_STATE_CONTROL_TAG
...
class NoisyCircuitCreator:
    def __init__(self, circuit: Circuit, num_data_qubits: int):
        ...
        self._measurement_operation = defaultdict(lambda: FastMeasurement())
    ...
    def _get_noisy_operation(self, operation: Operation) -> list[Operation]:
        # TODO get this working. break it into small circuit tests
        # fast_measurement_noise = self._get_fast_measurement_noise(operation)
    ...
    def _get_fast_measurement_noise(self, operation: Operation) -> Optional[list[Operation]]:
        if OPERATIONS_APPLIER_USING_CAT_STATE_CONTROL_TAG in operation.tags:
            ...
            uid = next(tag for tag in operation.tags if OPERATIONS_APPLIER_USING_CAT_STATE_CONTROL_TAG != tag)
        ...
        elif FAULT_TOLERANT_MEASURER_FAST_MEASUREMENT_TAG in operation.tags:
            uid = next(tag for tag in operation.tags if FAULT_TOLERANT_MEASURER_FAST_MEASUREMENT_TAG != tag)
            self._measurement_operation[uid].count -= 1
