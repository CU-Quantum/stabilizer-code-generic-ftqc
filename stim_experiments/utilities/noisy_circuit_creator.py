...
from stim_experiments.custom_dataclasses.noisy_operations_count import NoisyOperationsCount, NoisyOperationsCountPerGate
...
class NoisyChannel(Gate, ABC):
    ...
    def set_noisy_operations_count(self, noisy_operations_count: NoisyOperationsCount) -> None:
...
class NoisyCircuitCreator:
    ...
    def _count_noisy_ops_between_tags(self, op_tree: OP_TREE) -> NoisyOperationsCountPerGate:
        circuit = Circuit(op_tree)
        count = NoisyOperationsCountPerGate()
        for op in circuit.all_operations():
            ...
            if CORRECTION_ROUND_TAG in op.tags:
                ...
                op.gate.set_noisy_operations_count(count.latest_count)
                if op.gate.noisy_channel_type == NoisyChannelType.ONE:
                    count.latest_count.one_qubit += 1
                elif op.gate.noisy_channel_type == NoisyChannelType.TWO:
